	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "欢迎" : "手柄输入设置",
	...
	if(numDevices > 0)
		deviceInfo << numDevices << " 个手柄" << (numDevices > 1 ? "S" : "") << " 被识别";
	else
		deviceInfo << "没有检测到任何手柄";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "请按住您想设置的手柄上的任意按键.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "按F4键可随时退出." : "按键盘上的ESC键可取消.";
