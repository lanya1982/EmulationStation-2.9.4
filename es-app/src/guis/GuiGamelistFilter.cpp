...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "重置全部过滤", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addButton("返回", "back", std::bind(&GuiGamelistFilter::applyFilters, this));
