	...
	mClose(false)
{
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ÊäÈë", "search", [&] {
		mSearch->openInputScreen(mSearchParams);
		mGrid.resetCursor();
	}));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "È¡Ïû", "cancel", [&] { delete this; }));
