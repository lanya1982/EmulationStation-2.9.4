	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "编辑元信息", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(mDeleteFunc)
	{
		...
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "删除", "delete", deleteBtnFunc));
	}
